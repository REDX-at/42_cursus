...
static bool is_dead(t_philo *philo)
{
    ...
    if (get_bool(&philo->philo_mutex, &philo->full))
        return false;
    time_passed = gettime(MILLISECOND) - get_long(&philo->philo_mutex, &philo->last_meal);
...
void	*watch_dinner(void *data)
{
	...
	table = (t_table *)data;
	while (!all_threads_running(&table->DataMutex, &table->thread_exe_nbr, table->philo_nbr))
		;
